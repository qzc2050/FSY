...
{
  return HAL_I2C_Mem_Read(&hi2c1, BMP280_I2C_ADDR, reg, I2C_MEMADD_SIZE_8BIT, buf, len, 50U);
}

static HAL_StatusTypeDef BMP280_WriteReg(uint8_t reg, uint8_t value)
{
  return HAL_I2C_Mem_Write(&hi2c1, BMP280_I2C_ADDR, reg, I2C_MEMADD_SIZE_8BIT, &value, 1U, 50U);
}
...
void BMP280_GetData(BMP280_Data_t *out)
{
  ...
  *out = s_bmp280;
  ...
  now = HAL_GetTick();
  diff = now - s_bmp280.last_update_tick;
