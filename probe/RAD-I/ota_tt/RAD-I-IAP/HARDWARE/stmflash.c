...
{ 
    ...
    if(bank1_words > 0)    // 写 Bank1 部分（如果有）
    {
        ...
        // 擦除 Bank1 扇区（仅覆盖本次写入范围，勿扫到 BANK1_END_ADDR）
        {
            uint32_t erase_end = (endaddr < BANK1_END_ADDR) ? endaddr : BANK1_END_ADDR;
            while(addrx < erase_end)
            {
...
//        }
//        res = Reg_Flash_GetErrorStatus(idx_bank);
//        if(res) FLASH->CCR2 = 0X07EE0000;		//清所有错误标志
//        return res;
//    }
//}
...
////pBuffer:数据指针
////NumToWrite:字 (32 位) 数
//void Reg_Flash_Write(uint32_t WriteAddr, uint32_t *pBuffer, uint32_t NumToWrite)	
//{ 
//    #define MAX_ONCE_WRITE_WORD  8
//    
////    uint8_t res;
...
//    }
//    
//    while(Reg_Flash_WaitDone(bankx, 0XFFFFFFFF));
//    
//    // 写 Bank1 部分（如果有）
//    if(bank1_words > 0)
//    {
...
////测试写入
//void Reg_Flash_Test_Write(uint32_t WriteAddr, uint32_t WriteData)   	
//{
//	Reg_Flash_Write(WriteAddr, &WriteData, 1);
//}
