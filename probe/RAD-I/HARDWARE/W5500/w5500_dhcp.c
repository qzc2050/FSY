...
#include "../dev_protocol/net_raw/net_raw_bsp.h"
...
static void W5500_DHCP_Network_Recover(void)
{
    ...
    getSIPR(ip);
    printf("[DHCP-WDG] IP=%u.%u.%u.%u 链路 UP 但无效，复位 W5500 并重初始化...\r\n",
    ...
    getSIPR(ip);
    printf("[DHCP-WDG] W5500 已复位，重新 DHCP，当前 IP=%u.%u.%u.%u\r\n",
           (unsigned)ip[0], (unsigned)ip[1], (unsigned)ip[2], (unsigned)ip[3]);
}
...
void W5500_DHCP_Init(void)
{
    ...
    {
        printf("[DHCP] WIZnet DHCP 库已初始化\r\n");
        printf("[DHCP] 正在获取动态 IP 地址...\r\n");
...
static void DHCP_Check_Link_Status(void)
{
    ...
    if(current_link != last_link_status)
    {
        ...
        if(current_link)
        {
            printf("[DHCP] 网络链路已连接，触发 DHCP 请求\r\n");
            /* 链路连接，重新获取 DHCP */
            g_dhcp_state = DHCP_STATE_INIT;
            g_dhcp_retry_cnt = 0;
            /* 重置内部 DHCP 状态机，确保同步 */
...
void UDP_Broadcast_Init(void)
{
    ...
    if(ret == 1)
    {
        ...
        printf("[UDP] 组播 Socket %d -> 目标 %u.%u.%u.%u:%u\r\n",
...
uint16_t Broadcast_Fill_Data(uint8_t *buf)
{
    ...
    for (i = 0U; i < (size_t)DEVICE_CFG_SN_LEN; i++)
        sn_safe[i] = sys_cfg.SN[i];
    sn_safe[DEVICE_CFG_SN_LEN] = '\0';

    n = snprintf((char *)buf, (size_t)UDP_DISCOVER_PAYLOAD_MAX,
                 "%s,%s,%u.%u.%u.%u,%u,%u,%u,%u,%u",
                 DEVICE_PRODUCT_MODEL,
                 sn_safe,
                 (unsigned)ip[0], (unsigned)ip[1], (unsigned)ip[2], (unsigned)ip[3],
...
void UDP_Broadcast_Task(void)
{
    ...
    if(!W5500_Is_IP_Valid())
    {
        ...
        {
            printf("[广播] IP 地址无效 (%u.%u.%u.%u)，等待 DHCP 获取有效 IP...\r\n",
                   (unsigned)ip[0], (unsigned)ip[1], (unsigned)ip[2], (unsigned)ip[3]);
    ...
    {
        printf("[UDP] IP 已生效 (%u.%u.%u.%u)，重新绑定 UDP 组播 Socket\r\n",
                (unsigned)ip[0], (unsigned)ip[1], (unsigned)ip[2], (unsigned)ip[3]);
...
DHCP_State_t DHCP_Client_Task(void)
{
    ...
    if(dhcp_ret == DHCP_OK)
    {
           ...
           (ip[2] != s_tcp_bound_ip[2]) || (ip[3] != s_tcp_bound_ip[3]))
        {
            if(g_dhcp_state != DHCP_STATE_BOUND)
            {
                printf("[DHCP] 成功获取到 DHCP IP 地址\r\n");
                printf("[DHCP] 当前 IP: %u.%u.%u.%u，重新绑定 UDP 组播 Socket\r\n",
                       (unsigned)ip[0], (unsigned)ip[1], (unsigned)ip[2], (unsigned)ip[3]);
            }
            else
            {
                printf("[DHCP] IP 变更: %u.%u.%u.%u -> %u.%u.%u.%u，重建网络 Socket\r\n",
