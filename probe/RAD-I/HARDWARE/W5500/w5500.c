...
/*
 * W5500 SPI ���ٽ磺�� ulPortRaiseBASEPRI / �ָ��������� taskENTER_CRITICAL��
 * ������ vPortEnterCritical ���� uxCriticalNesting������ printf/��/�����ٽ���
 * �����λ������ vPortExitCritical(port.c:450)��
 */
...
uint16 wiz_read_buf(uint32 addrbsb, uint8* buf,uint16 len)
{
  ...
  IINCHIP_ISR_DISABLE();
  IINCHIP_CSoff();
  IINCHIP_SpiSendData( (addrbsb & 0x00FF0000)>>16);		// ͨ��SPI����16λ��ַ�θ�MCU
  IINCHIP_SpiSendData( (addrbsb & 0x0000FF00)>> 8);		// 
  IINCHIP_SpiSendData( (addrbsb & 0x000000F8));    		// ����SPIΪ������
  for(idx = 0; idx < len; idx++)                    	// ��buf�е�����ͨ��SPI���͸�MCU
  {
...
// W5500��8��Socket��ÿ��Socket�ж�Ӧ�������շ���������
// ÿ��Socket�ķ���/���ջ���������һ��16KB�����������ڴ��У���ʼ������Ϊ2KB��
// ���۸�ÿ��Socket���������/�����棬��������16KB���ڡ�

    if (ssum <= 16384)										// ����Socket���ͻ���ռ�Ĵ�С
    {
       switch( tx_size[i] )
				{
					case 1:
					...
					break;
					default :
						RSIZE[i] = (int16)(2048);			// Ĭ��i=2��tx_size=2KB
					break;
				}
		}

			if (rsum <= 16384)									// ����Socket���ջ���ռ�Ĵ�С
			{
					switch( rx_size[i] )				
					{
						case 1:
						...
						break;
						default :
							RSIZE[i] = (int16)(2048);		// Ĭ��i=2��rx_size=2K
						break;
					}
			}
    ssum += SSIZE[i];
...
*/
uint16 getSn_RX_RSR(SOCKET s)														// ��ȡ���н��ջ���Ĵ�����ֵ
{
  uint16 val=0,val1=0;
  do
  {
    val1 = IINCHIP_READ(Sn_RX_RSR0(s));									// MCU��Sn_RX_RSR�ĵ�8λ��������val1
    val1 = (val1 << 8) + IINCHIP_READ(Sn_RX_RSR1(s));		// ����8λ�������8λ��Ӹ���val1
    if(val1 != 0)																				// ��Sn_RX_RSR��ֵ��Ϊ0�����丳��val
    {
        val = IINCHIP_READ(Sn_RX_RSR0(s));
        val = (val << 8) + IINCHIP_READ(Sn_RX_RSR1(s));
    }
  } while (val != val1);																// �ж�val��val1�Ƿ���ȣ������ȣ����·���doѭ��������ȣ�����ѭ��
   return val;																					// ��val��ֵ���ظ�getSn_RX_RSR
}
...
void recv_data_processing(SOCKET s, uint8 *data, uint16 len)
{
  ...
  uint32 addrbsb = 0;
	...
	// MCU��ȡSn_RX_RD����дָ��Ĵ�����ֵ��������ptr
	// Sn_RX_RD������ջ��������ݵ��׵�ַ���������ݽ��գ���������üĴ���ֵҪ����
  ptr = IINCHIP_READ( Sn_RX_RD0(s) );
  ptr = ((ptr & 0x00ff) << 8) + IINCHIP_READ( Sn_RX_RD1(s) );
	
  addrbsb = (uint32)(ptr<<8) + (s<<5) + 0x18;		// ��ȡ���յ������ݵľ��Ե�ַ
  wiz_read_buf(addrbsb, data, len);							// ͨ�����Ե�ַ�������յ������ݷ���MCU
