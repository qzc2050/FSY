...
/**
  ...
  *         1: 无应答
  */
uint8_t pcf8563_iic_send_byte(uint8_t txd)
{
    return I2C1_Master_Transmit(PCF8563_WRITE_ADDR, &txd, 1, I2C_BUS_TIMEOUT_MS) != HAL_OK;
}
...
/**
  ...
  * @retval 读取到的数据
  */
uint8_t pcf8563_iic_read_byte(uint8_t ack)
{
    uint8_t receive_data = 0;
    
    // 使用HAL库读取数据，ack参数在此实现中主要用于内部处理
    if (I2C1_Master_Receive(PCF8563_READ_ADDR, &receive_data, 1, I2C_BUS_TIMEOUT_MS) != HAL_OK) {
...
{
    uint8_t data[2] = {reg, reg_val};
    
    if (I2C1_Master_Transmit(PCF8563_WRITE_ADDR, data, 2, I2C_BUS_TIMEOUT_MS) != HAL_OK) {
        return 1; // 传输失败
    }
...
/**
  * @brief  从PCF8563指定寄存器读取一个字节
  * @param  reg: 要读取的寄存器地址
  * @retval 读取到的数据，0xFF表示读取失败
  */
uint8_t pcf8563_read_byte(uint8_t reg)
{
    ...
    // 先发送寄存器地址
    if (I2C1_Master_Transmit(PCF8563_WRITE_ADDR, &reg, 1, I2C_BUS_TIMEOUT_MS) != HAL_OK) {
        return 0xFF;
    }
...
uint8_t pcf8563_get_time(struct time_type__ *p)
{
    ...
    // 读取各时间寄存器，并去掉无效位
    local_time.year   = pcf8563_read_byte(0x0A); // 年
    local_time.month  = pcf8563_read_byte(0x09) & 0x1F; // 月
    local_time.week   = pcf8563_read_byte(0x08) & 0x07; // 周
    local_time.day    = pcf8563_read_byte(0x07) & 0x3F; // 日
    local_time.hour   = pcf8563_read_byte(0x06) & 0x3F; // 时
    local_time.minute = pcf8563_read_byte(0x05) & 0x7F; // 分
    local_time.second = pcf8563_read_byte(0x04) & 0x7F; // 秒
...
/**
  * @brief  设置PCF8563的时间
  * @param  p: 包含要设置的时间的结构体指针
  */
void pcf8563_set_time(struct time_type__ *p)
{
    pcf8563_send_byte(0x0A, p->year);    // 年
    pcf8563_send_byte(0x09, p->month);  // 月
    pcf8563_send_byte(0x08, p->week);   // 周
    pcf8563_send_byte(0x07, p->day);    // 日
    pcf8563_send_byte(0x06, p->hour);   // 时
    pcf8563_send_byte(0x05, p->minute); // 分
    pcf8563_send_byte(0x04, p->second); // 秒
}

/**
  * @brief  设置PCF8563的控制寄存器
  * @param  val: 要设置的值
  */
void pcf8563_set_cap(uint8_t val)
{
    ...
    reg_val = pcf8563_read_byte(0x00); // 读取控制寄存器
    ...
    pcf8563_send_byte(0x00, reg_val); // 写回控制寄存器
}
...
{
	return (date_time.year * 10000 + date_time.month * 100 + date_time.day);
}
...
//    LPR_Critical_Execute(pcf8563_get_cur_time,&date_time);

//	if((minute != date_time.minute) || ref)
//	{
//		cheak_date(1);
...
//        sprintf(str_temp,"20%02d/%02d/%02d %02d:%02d",date_time.year,date_time.month,date_time.day,\
//                                                date_time.hour,date_time.minute);
//		
////		if((crt_inft == MENU_HOME_1) || (crt_inft == TIMING_RUN) || (crt_inft == TIMING_SW))
//		OLED_ShowString(2,1,(uint8_t *)str_temp,12);
////		printf("时钟%d年%d月%d日 星期:%d   %d:%d:%d\r\n", date_time.year + 2000, date_time.month, date_time.day, date_time.week, date_time.hour, date_time.minute, date_time.second);
//	}
}
