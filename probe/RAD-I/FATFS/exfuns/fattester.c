...
//格式化
//path:磁盘路径，比如"0:"、"1:"
//opt:模式，FM_FAT,FM_FAT32,FM_EXFAT,FM_ANY 等...
//au:簇大小
//返回值：执行结果
uint8_t mf_fmkfs(uint8_t* path,uint8_t opt,uint16_t au)
{ 
	return f_mkfs((const TCHAR*)path,opt,au,fatbuf,FF_MAX_SS);//格式化，drv:盘符;opt:模式;au:簇大小，workbuf，最少_MAX_SS 大小
} 
//删除文件/目录
//pname:文件/目录路径 + 名字
//返回值：执行结果
...
//设置盘符（磁盘名字），最长 11 个字符！！，支持数字和大写字母组合以及汉字等
