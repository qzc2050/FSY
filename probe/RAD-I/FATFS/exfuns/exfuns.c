...
//		fs[i]=(FATFS*)osRtxMemoryAlloc(MALLOC_AXISRAM, sizeof(FATFS), 0);
...
//	file = (FIL*)osRtxMemoryAlloc(MALLOC_AXISRAM, sizeof(FIL), 0);
//	ftemp = (FIL*)osRtxMemoryAlloc(MALLOC_AXISRAM, sizeof(FIL), 0);
//	fatbuf = (uint8_t*)osRtxMemoryAlloc(MALLOC_AXISRAM, 512, 0);
	file = (FIL*)ff_memalloc(sizeof(FIL));
	ftemp = (FIL*)ff_memalloc(sizeof(FIL));
	fatbuf = (uint8_t*)ff_memalloc(512);
...
//	fsrc=(FIL*)osRtxMemoryAlloc(MALLOC_AXISRAM, sizeof(FIL), 0);
//	fdst=(FIL*)osRtxMemoryAlloc(MALLOC_AXISRAM, sizeof(FIL), 0);
//	fbuf=(uint8_t*)osRtxMemoryAlloc(MALLOC_AXISRAM, 8192, 0);
...
//	osRtxMemoryFree(MALLOC_AXISRAM, fsrc);
//	osRtxMemoryFree(MALLOC_AXISRAM, fdst);
//	osRtxMemoryFree(MALLOC_AXISRAM, fbuf);
	ff_memfree(fsrc);
	ff_memfree(fdst);
	ff_memfree(fbuf);
...
//	fddir=(DIR*)osRtxMemoryAlloc(MALLOC_AXISRAM, sizeof(DIR), 0);
	fddir=(DIR*)ff_memalloc(sizeof(DIR));

 	//finfo=(FILINFO*)mymalloc(SRAMIN,sizeof(FILINFO));
//	finfo=(FILINFO*)osRtxMemoryAlloc(MALLOC_AXISRAM, sizeof(FILINFO), 0);
...
//		pathname=osRtxMemoryAlloc(MALLOC_AXISRAM, MAX_PATHNAME_DEPTH, 0);
...
//			osRtxMemoryFree(MALLOC_AXISRAM, pathname);
			ff_memfree(pathname);
		}
 	}
	//myfree(SRAMIN,fddir);    
...
//	osRtxMemoryFree(MALLOC_AXISRAM, fddir);
//	osRtxMemoryFree(MALLOC_AXISRAM, finfo);
	ff_memfree(fddir);
	ff_memfree(finfo);
...
//	srcdir=(DIR*)osRtxMemoryAlloc(MALLOC_AXISRAM, sizeof(DIR), 0);
//	dstdir=(DIR*)osRtxMemoryAlloc(MALLOC_AXISRAM, sizeof(DIR), 0);
//	finfo=(FILINFO*)osRtxMemoryAlloc(MALLOC_AXISRAM, sizeof(FILINFO), 0);
...
//		dstpathname = osRtxMemoryAlloc(MALLOC_AXISRAM, MAX_PATHNAME_DEPTH, 0);
//		srcpathname = osRtxMemoryAlloc(MALLOC_AXISRAM, MAX_PATHNAME_DEPTH, 0);
...
//			osRtxMemoryFree(MALLOC_AXISRAM, dstpathname);
//			osRtxMemoryFree(MALLOC_AXISRAM, srcpathname);
...
//	osRtxMemoryFree(MALLOC_AXISRAM, srcdir);
//	osRtxMemoryFree(MALLOC_AXISRAM, dstdir);
//	osRtxMemoryFree(MALLOC_AXISRAM, finfo);
	ff_memfree(srcdir);
	ff_memfree(dstdir);
	ff_memfree(finfo);
