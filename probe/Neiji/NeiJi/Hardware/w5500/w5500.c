...
SPI??1??SPI??2???w5500.h ?????
#define MODE_SPI  1     // ???SPI1  
#define MODE_SPI  2     // ???SPI2
?????????
***********************************************************************************/
...
static osMutexId_t s_w5500_spi_mutex;
static const osMutexAttr_t s_w5500_spi_mutex_attr = {
    .name = "w5500Spi",
};
...
void W5500_SpiMutexInit(void)
{
    if (s_w5500_spi_mutex == NULL) {
        s_w5500_spi_mutex = osMutexNew(&s_w5500_spi_mutex_attr);
    }
}
...
{
    if (s_w5500_spi_mutex != NULL) {
        (void)osMutexAcquire(s_w5500_spi_mutex, osWaitForever);
    }
}
...
{
    if (s_w5500_spi_mutex != NULL) {
        (void)osMutexRelease(s_w5500_spi_mutex);
    }
}

static uint32_t s_spi_ok;
static uint32_t s_spi_err;
static uint32_t s_spi_last_hal_st;
static uint32_t s_spi_last_err_code;
...
void W5500_SpiRecover(void)
{
    ...
    if (hspi1.Instance != NULL) {
        SET_BIT(hspi1.Instance->IFCR,
                SPI_IFCR_EOTC | SPI_IFCR_OVRC | SPI_IFCR_MODFC |
                SPI_IFCR_TIFREC | SPI_IFCR_UDRC | SPI_IFCR_TXTFC);
    }
...
void W5500_SpiGetStats(uint32_t *ok, uint32_t *err, uint32_t *last_st, uint32_t *last_ecode)
{
    ...
    if (last_st != NULL) {
        *last_st = s_spi_last_hal_st;
    }
    if (last_ecode != NULL) {
        *last_ecode = s_spi_last_err_code;
    }
}
...
uint8_t W5500_SpiRawReadVersion(void)
{
    ...
    rx[0] = IINCHIP_SpiSendData(tx[0]);
    rx[1] = IINCHIP_SpiSendData(tx[1]);
    rx[2] = IINCHIP_SpiSendData(tx[2]);
    rx[3] = IINCHIP_SpiSendData(tx[3]);
    IINCHIP_CSon();
    IINCHIP_ISR_ENABLE();

    printf("[W5500] raw SPI rx: %02X %02X %02X %02X\r\n",
           rx[0], rx[1], rx[2], rx[3]);
    return rx[3];
}
...
uint8_t  IINCHIP_SpiSendData(uint8 dat)
{
    ...
    st = HAL_SPI_TransmitReceive(&hspi1, &dat, &read_data, 1, 100U);
    if (st == HAL_OK) {
        s_spi_ok++;
    } else {
        s_spi_err++;
        s_spi_last_hal_st = (uint32_t)st;
        s_spi_last_err_code = (uint32_t)hspi1.ErrorCode;
        W5500_SpiRecover();
        read_data = 0U;
    }
    return read_data;
...
uint16 wiz_read_buf(uint32 addrbsb, uint8* buf,uint16 len)
{
  ...
  IINCHIP_ISR_DISABLE();
  IINCHIP_CSoff();
  IINCHIP_SpiSendData( (addrbsb & 0x00FF0000)>>16);		// ???SPI????16?????????MCU
  IINCHIP_SpiSendData( (addrbsb & 0x0000FF00)>> 8);		// 
  IINCHIP_SpiSendData( (addrbsb & 0x000000F8));    		// ????SPI???????
  for(idx = 0; idx < len; idx++)                    	// ??buf???????????SPI?????MCU
  {
...
// W5500??8??Socket?????Socket??????????????????????
// ???Socket?????/????????????????16KB???????????????????????????2KB??
// ????????Socket?????????/???????????????16KB?????

    if (ssum <= 16384)										// ????Socket?????????????
    {
       switch( tx_size[i] )
				{
					case 1:
					...
					break;
					default :
						RSIZE[i] = (int16)(2048);			// ???i=2??tx_size=2KB
					break;
				}
		}

			if (rsum <= 16384)									// ????Socket?????????????
			{
					switch( rx_size[i] )				
					{
						case 1:
						...
						break;
						default :
							RSIZE[i] = (int16)(2048);		// ???i=2??rx_size=2K
						break;
					}
			}
    ssum += SSIZE[i];
...
/**
@brief  W5500??????????????Socket???????????Sn_KPALVTR???????5s
*/
...
uint16 getSn_RX_RSR(SOCKET s)														// ?????????????????????
{
  ...
  do
  {
    val1 = IINCHIP_READ(Sn_RX_RSR0(s));									// MCU??Sn_RX_RSR???8??????????val1
    val1 = (val1 << 8) + IINCHIP_READ(Sn_RX_RSR1(s));		// ????8?????????8????????val1
    if(val1 != 0)																				// ??Sn_RX_RSR??????0??????P??val
    {
        val = IINCHIP_READ(Sn_RX_RSR0(s));
        val = (val << 8) + IINCHIP_READ(Sn_RX_RSR1(s));
    }
  } while (val != val1);																// ????val??val1??????????????????????do??????????????????
   return val;																					// ??val????????getSn_RX_RSR
}
...
void recv_data_processing(SOCKET s, uint8 *data, uint16 len)
{
  ...
  wiz_read_buf(addrbsb, data, len);							// ???????????????????????????MCU
