...
static void DHCP_Send_Discover(void)
{
    ...
    printf("[DHCP] send DISCOVER (len=%d)\r\n", msg_len);
}
...
static void DHCP_Send_Request(void)
{
    ...
    printf("[DHCP] send REQUEST (len=%d)\r\n", msg_len);
}
...
static int8_t DHCP_Parse_Response(uint8_t *buf, uint16_t len)
{
    ...
    if(len < 240)
    {
        printf("[DHCP] response too short: %d < 240\r\n", len);
        return DHCP_FAIL;
    }
    ...
    if(opt + 4 > opt_end)
    {
        printf("[DHCP] magic cookie out of range\r\n");
        return DHCP_FAIL;
    }
    if(opt[0] != 99 || opt[1] != 130 || opt[2] != 83 || opt[3] != 99)
    {
        printf("[DHCP] bad magic cookie: 0x%02x 0x%02x 0x%02x 0x%02x\r\n", 
    ...
    {
        printf("[DHCP] unknown msg type: 0x%02x\r\n", msg_type);
        return DHCP_FAIL;
    }
