...
#define W25Q_HAL_TIMEOUT_MS      500U
...
uint32_t W25Qx_QSPI_FLASH_ReadID(void)
{
	...
	if (HAL_QSPI_Command(&hqspi, &s_command, W25Q_HAL_TIMEOUT_MS) != HAL_OK)
		return 0U;
	if (HAL_QSPI_Receive(&hqspi, pData, W25Q_HAL_TIMEOUT_MS) != HAL_OK)
		return 0U;
